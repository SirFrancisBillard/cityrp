...
local function openПатроны()
	if IsValid(essentialDarkRPF4Menu.ПатроныPanel) then return end

	local midPanel = essentialDarkRPF4Menu.returnMidPanel()

	-- Panel to parent all tab content to
	essentialDarkRPF4Menu.ПатроныPanel = midPanel:Add('DPanel')
	essentialDarkRPF4Menu.ПатроныPanel:SetSize(midPanel:GetWide(), midPanel:GetTall())
	essentialDarkRPF4Menu.ПатроныPanel:Dock(FILL)
	essentialDarkRPF4Menu.ПатроныPanel:DockMargin(0, 0, 0, 0)

	function essentialDarkRPF4Menu.ПатроныPanel:Paint() return end

	-- Right panel
	local rightPanel = essentialDarkRPF4Menu.ПатроныPanel:Add('DPanel')
	rightPanel:SetSize(270, essentialDarkRPF4Menu.ПатроныPanel:GetTall())
	...
	local function addAmmoPrice()
		if essentialDarkRPF4Menu.selectedAmmo.price < 1 then
			descriptionPrice:SetText('Free')
		else
			descriptionPrice:SetText('Cost: ' .. DarkRP.formatMoney(essentialDarkRPF4Menu.selectedAmmo.price))
		end
	end
	...
	purchaseAmmoButton:SetText('Purchase')
	...
	-- Ammo list panel
	local ammoListPanel = essentialDarkRPF4Menu.ПатроныPanel:Add('DScrollPanel')
	ammoListPanel:SetSize(essentialDarkRPF4Menu.ПатроныPanel:GetWide(), essentialDarkRPF4Menu.ПатроныPanel:GetTall())
	...
	for _, ammo in ipairs(GAMEMODE.AmmoTypes) do
		...
		if essentialDarkRPF4Menu.displayItem then
			...
			if ammo.price < 1 then
				ammoPrice:DockMargin(4, 0, 0, 0)
				ammoPrice:SetText('Free')
			else
				ammoPrice:DockMargin(0, 0, 0, 0)
	...
	if #validItems < 1 then
		...
		local unavailableLabel = essentialDarkRPF4Menu.ПатроныPanel:Add('DLabel')
...
function essentialDarkRPF4Menu.openПатроныTab()
	openПатроны()
end
