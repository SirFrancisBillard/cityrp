...
local function openЯщики()
	if IsValid(essentialDarkRPF4Menu.ЯщикиPanel) then return end

	local midPanel = essentialDarkRPF4Menu.returnMidPanel()

	-- Panel to parent all tab content to
	essentialDarkRPF4Menu.ЯщикиPanel = midPanel:Add('DPanel')
	essentialDarkRPF4Menu.ЯщикиPanel:SetSize(midPanel:GetWide(), midPanel:GetTall())
	essentialDarkRPF4Menu.ЯщикиPanel:Dock(FILL)
	essentialDarkRPF4Menu.ЯщикиPanel:DockMargin(0, 0, 0, 0)

	function essentialDarkRPF4Menu.ЯщикиPanel:Paint() return end

	-- Right panel
	local rightPanel = essentialDarkRPF4Menu.ЯщикиPanel:Add('DPanel')
	rightPanel:SetSize(270, essentialDarkRPF4Menu.ЯщикиPanel:GetTall())
	...
	descriptionType:SetText('Shipment')
	...
	local function addShipmentPrice()
		if essentialDarkRPF4Menu.selectedShipment.price < 1 then
			descriptionPrice:SetText('Free')
		else
			descriptionPrice:SetText('Cost: ' .. DarkRP.formatMoney(essentialDarkRPF4Menu.selectedShipment.price))
		end
	end
	...
	local function addShipmentAmount()
		descriptionAmount:SetText('Holds: ' .. essentialDarkRPF4Menu.selectedShipment.amount)
	end
	...
	purchaseShipmentButton:SetText('Purchase')
	...
	local shipmentsListPanel = essentialDarkRPF4Menu.ЯщикиPanel:Add('DScrollPanel')
	shipmentsListPanel:SetSize(essentialDarkRPF4Menu.ЯщикиPanel:GetWide(), essentialDarkRPF4Menu.ЯщикиPanel:GetTall())
	...
	for _, shipment in ipairs(CustomShipments) do
		if not shipment.noship then
			...
			if essentialDarkRPF4Menu.displayItem then
				...
				if shipment.price < 1 then
					shipmentPrice:DockMargin(4, 0, 0, 0)
					shipmentPrice:SetText('Free')
				else
					shipmentPrice:DockMargin(0, 0, 0, 0)
	...
	if #validItems < 1 then
		...
		local unavailableLabel = essentialDarkRPF4Menu.ЯщикиPanel:Add('DLabel')
...
function essentialDarkRPF4Menu.openЯщикиTab()
	openЯщики()
end
