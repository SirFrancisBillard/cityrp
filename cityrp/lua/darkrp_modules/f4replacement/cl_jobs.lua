...
local function openРабота()
	if IsValid(essentialDarkRPF4Menu.РаботаPanel) then return end

	local midPanel = essentialDarkRPF4Menu.returnMidPanel()

	-- Panel to parent all tab content to
	essentialDarkRPF4Menu.РаботаPanel = midPanel:Add('DPanel')
	essentialDarkRPF4Menu.РаботаPanel:SetSize(midPanel:GetWide(), midPanel:GetTall())
	essentialDarkRPF4Menu.РаботаPanel:Dock(FILL)
	essentialDarkRPF4Menu.РаботаPanel:DockMargin(0, 0, 0, 0)

	function essentialDarkRPF4Menu.РаботаPanel:Paint() return end

	-- Right panel
	local rightPanel = essentialDarkRPF4Menu.РаботаPanel:Add('DPanel')
	rightPanel:SetSize(270, essentialDarkRPF4Menu.РаботаPanel:GetTall())
	...
	changeJobButton:SetText('Take Job')
	...
	function changeJobButton:Paint(w, h)
		...
		if essentialDarkRPF4Menu.selectedJob.vote then
			changeJobButton:SetText('Create Vote')
		else
			changeJobButton:SetText('Take Job')
		end
	...
	-- Jobs list panel
	local jobsListPanel = essentialDarkRPF4Menu.РаботаPanel:Add('DScrollPanel')
	jobsListPanel:SetSize(essentialDarkRPF4Menu.РаботаPanel:GetWide(), essentialDarkRPF4Menu.РаботаPanel:GetTall())
	...
	for _, job in ipairs(RPExtraTeams) do
		...
		if essentialDarkRPF4Menu.displayItem then
			...
			if job.salary < 1 then
				jobSalary:DockMargin(4, 0, 0, 0)
				jobSalary:SetText('None')
			else
				jobSalary:DockMargin(0, 0, 0, 0)
				jobSalary:SetText('$' .. job.salary)
			end
...
function essentialDarkRPF4Menu.openРаботаTab()
	openРабота()
end
