...
local buttonNormal = Color(10, 10, 10, 140)
local buttonTab = Color(10, 10, 10, 160)
...
local function drawTabsPanel()
	...
	function tabsHeader:Paint()
		drawPanelOutlined(self, buttonNormal)
	end
	...
	if #RPExtraTeams > 0 then
		...
		table.insert(tabs, essentialDarkRPF4Menu.tabPos, 'Работа')
	end

	if #DarkRPEntities > 0 then
		essentialDarkRPF4Menu.tabPos = essentialDarkRPF4Menu.tabPos + 1

		table.insert(tabs, essentialDarkRPF4Menu.tabPos, 'Предметы')
	end

	if #CustomShipments > 0 then
		essentialDarkRPF4Menu.tabPos = essentialDarkRPF4Menu.tabPos + 1

		table.insert(tabs, essentialDarkRPF4Menu.tabPos, 'Ящики')

		essentialDarkRPF4Menu.tabPos = essentialDarkRPF4Menu.tabPos + 1

		table.insert(tabs, essentialDarkRPF4Menu.tabPos, 'Оружие')
	end

	if #GAMEMODE.AmmoTypes > 0 then
		essentialDarkRPF4Menu.tabPos = essentialDarkRPF4Menu.tabPos + 1

		table.insert(tabs, essentialDarkRPF4Menu.tabPos, 'Патроны')
	end
	...
	if not DarkRP.disabledDefaults['modules']['hungermod'] then
		...
		table.insert(tabs, essentialDarkRPF4Menu.tabPos, 'Еда')
	end
	...
	function closeButton:Paint()
		...
		surface.SetDrawColor(buttonNormal)
...
local function drawMainFrameHeader()
	...
	mainFrameHeader:SetSize(300, 58)

	function mainFrameHeader:Paint(w, h)
		surface.SetDrawColor(defaultBlurOutline)
...
local function openF4MenuToggle()
	if IsValid(essentialDarkRPF4Menu.mainFrame) then
		...
		if essentialDarkRPF4Menu.rememberedTab == nil then
			...
			essentialDarkRPF4Menu.openРаботаTab()
		else
...
hook.Add('ShowSpare2', 'edf_openF4Menu', function()
	...
	-- Stop drawing default DarkRP F4 Menu
	return false
end)
