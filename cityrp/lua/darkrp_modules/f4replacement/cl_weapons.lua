...
local function openОружие()
	if IsValid(essentialDarkRPF4Menu.ОружиеPanel) then return end

	local midPanel = essentialDarkRPF4Menu.returnMidPanel()

	-- Panel to parent all tab content to
	essentialDarkRPF4Menu.ОружиеPanel = midPanel:Add('DPanel')
	essentialDarkRPF4Menu.ОружиеPanel:SetSize(midPanel:GetWide(), midPanel:GetTall())
	essentialDarkRPF4Menu.ОружиеPanel:Dock(FILL)
	essentialDarkRPF4Menu.ОружиеPanel:DockMargin(0, 0, 0, 0)

	function essentialDarkRPF4Menu.ОружиеPanel:Paint() return end

	-- Right panel
	local rightPanel = essentialDarkRPF4Menu.ОружиеPanel:Add('DPanel')
	rightPanel:SetSize(270, essentialDarkRPF4Menu.ОружиеPanel:GetTall())
	...
	descriptionType:SetText('Separate')
	...
	local function addWeaponPrice()
		if essentialDarkRPF4Menu.selectedWeapon.pricesep < 1 then
			descriptionPrice:SetText('Free')
		else
			descriptionPrice:SetText('Cost: ' .. DarkRP.formatMoney(essentialDarkRPF4Menu.selectedWeapon.pricesep))
		end
	end
	...
	purchaseWeaponButton:SetText('Purchase')
	...
	-- Weapons list panel
	local weaponsListPanel = essentialDarkRPF4Menu.ОружиеPanel:Add('DScrollPanel')
	weaponsListPanel:SetSize(essentialDarkRPF4Menu.ОружиеPanel:GetWide(), essentialDarkRPF4Menu.ОружиеPanel:GetTall())
	...
	for _, weapon in ipairs(CustomShipments) do
		if weapon.separate then
			...
			if essentialDarkRPF4Menu.displayItem then
				...
				if weapon.pricesep < 1 then
					weaponPrice:DockMargin(4, 0, 0, 0)
					weaponPrice:SetText('Free')
				else
					weaponPrice:DockMargin(0, 0, 0, 0)
	...
	if #validItems < 1 then
		...
		local unavailableLabel = essentialDarkRPF4Menu.ОружиеPanel:Add('DLabel')
...
function essentialDarkRPF4Menu.openОружиеTab()
	openОружие()
end
