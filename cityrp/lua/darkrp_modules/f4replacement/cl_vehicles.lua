...
local function openVehicles()
	...
	local function addVehiclePrice()
		if essentialDarkRPF4Menu.selectedVehicle.price < 1 then
			descriptionPrice:SetText('Free')
		else
			descriptionPrice:SetText('Cost: ' .. DarkRP.formatMoney(essentialDarkRPF4Menu.selectedVehicle.price))
		end
	end
	...
	purchaseVehicleButton:SetText('Purchase')
	...
	for _, vehicle in ipairs(CustomVehicles) do
		...
		if essentialDarkRPF4Menu.displayItem then
			...
			if vehicle.price < 1 then
				vehiclePrice:DockMargin(4, 0, 0, 0)
				vehiclePrice:SetText('Free')
			else
				vehiclePrice:DockMargin(0, 0, 0, 0)
