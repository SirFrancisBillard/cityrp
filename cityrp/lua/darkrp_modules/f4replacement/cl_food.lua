...
local function openЕда()
	if IsValid(essentialDarkRPF4Menu.ЕдаPanel) then return end

	local midPanel = essentialDarkRPF4Menu.returnMidPanel()

	-- Panel to parent all tab content to
	essentialDarkRPF4Menu.ЕдаPanel = midPanel:Add('DPanel')
	essentialDarkRPF4Menu.ЕдаPanel:SetSize(midPanel:GetWide(), midPanel:GetTall())
	essentialDarkRPF4Menu.ЕдаPanel:Dock(FILL)
	essentialDarkRPF4Menu.ЕдаPanel:DockMargin(0, 0, 0, 0)

	function essentialDarkRPF4Menu.ЕдаPanel:Paint() return end

	-- Right panel
	local rightPanel = essentialDarkRPF4Menu.ЕдаPanel:Add('DPanel')
	rightPanel:SetSize(270, essentialDarkRPF4Menu.ЕдаPanel:GetTall())
	...
	local function addFoodEnergy()
		energyText:SetText('Energy: ' .. essentialDarkRPF4Menu.selectedFood.energy)
	end
	...
	local function addFoodPrice()
		if essentialDarkRPF4Menu.selectedFood.price < 1 then
			descriptionPrice:SetText('Free')
		else
			descriptionPrice:SetText('Cost: ' .. DarkRP.formatMoney(essentialDarkRPF4Menu.selectedFood.price))
		end
	end
	...
	purchaseFoodButton:SetText('Purchase')
	...
	-- Food list panel
	local foodListPanel = essentialDarkRPF4Menu.ЕдаPanel:Add('DScrollPanel')
	foodListPanel:SetSize(essentialDarkRPF4Menu.ЕдаPanel:GetWide(), essentialDarkRPF4Menu.ЕдаPanel:GetTall())
	...
	for _, food in ipairs(FoodItems) do
		...
		if essentialDarkRPF4Menu.displayItem then
			...
			if food.price < 1 then
				foodPrice:DockMargin(4, 0, 0, 0)
				foodPrice:SetText('Free')
			else
				foodPrice:DockMargin(0, 0, 0, 0)
	...
	if #validItems < 1 then
		...
		local unavailableLabel = essentialDarkRPF4Menu.ЕдаPanel:Add('DLabel')
...
function essentialDarkRPF4Menu.openЕдаTab()
